...
const ProductDetail = ({ productId }) => {
  ...
  const [mainImage, setMainImage] = useState('https://picsum.photos/500?random=1');
  ...
  useEffect(() => {
    if (selectedProduct?.images?.length > 0) {
      setMainImage(selectedProduct.images[0].url || 'https://picsum.photos/500?random=1');
    }
  }, [selectedProduct]);

  const handleQuantityChange = (action) => {
    setQuantity((prev) => (action === 'plus' ? Math.min(prev + 1, 5) : Math.max(prev - 1, 1)));
  };
            ...
            <div className="hidden md:flex flex-col space-y-4 mr-6">
              {selectedProduct.images.length > 0
                ? selectedProduct.images.map((image, index) => (
                    <img
                      key={index}
                      src={image.url || `https://picsum.photos/100?random=${index + 1}`}
            ...
            <div className="md:w-1/2 md:ml-10">
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center space-x-4 mt-2">
                  <button className="px-2 bg-gray-200 rounded text-lg" onClick={() => handleQuantityChange('minus')}>
                    -
                  </button>
                  <span className="text-lg">{quantity}</span>
                  <button className="px-2 bg-gray-200 rounded text-lg" onClick={() => handleQuantityChange('plus')}>
                    +
                  </button>
                </div>
              </div>
